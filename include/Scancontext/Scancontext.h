...
public:
    ...
    // loop thres
    // gil : search ration & dist_threshold for outdoor
    ...
    const double SC_DIST_THRES = 0.3; // empirically 0.1-0.2 is fine (rare false-alarms) for 20x60 polar context (but for 0.15 <, DCS or ICP fit score check (e.g., in LeGO-LOAM) should be required for robustness)
